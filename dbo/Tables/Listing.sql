CREATE TABLE [dbo].[Listing] (
    ...
    [IsDuplicate]          INT             CONSTRAINT [DEFAULT_Listing_IsDuplicate] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_Listing_ID] PRIMARY KEY NONCLUSTERED ([ID] ASC) WITH (FILLFACTOR = 80)
);


GO
CREATE UNIQUE CLUSTERED INDEX [IX_Listing_Code]
    ON [dbo].[Listing]([Code] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [IX_Listing_LastModifiedDateUTC]
    ON [dbo].[Listing]([LastModifiedDateUTC] ASC) WITH (FILLFACTOR = 80);
