CREATE TABLE [dbo].[Address_Invalid] (
    ...
    [IsDuplicate]                                  INT           CONSTRAINT [DEFAULT_Address_Invalid_IsDuplicate] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_Address_Invalid_ID] PRIMARY KEY NONCLUSTERED ([ID] ASC) WITH (FILLFACTOR = 80)
);


GO
CREATE CLUSTERED INDEX [IX_Address_Invalid_Code]
    ON [dbo].[Address_Invalid]([Code] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [IX_Address_Invalid_MADColumns]
    ON [dbo].[Address_Invalid]([IsMADSent] ASC, [MADSentDateUTC] ASC, [IsMADReceived] ASC, [MADReceivedDateUTC] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [IX_AddressInvalid_LastModifiedDateUTC]
    ON [dbo].[Address_Invalid]([LastModifiedDateUTC] DESC) WITH (FILLFACTOR = 80);
