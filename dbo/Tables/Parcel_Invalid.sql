CREATE TABLE [dbo].[Parcel_Invalid] (
    ...
    [IsDuplicate]                      INT             CONSTRAINT [DEFAULT_Parcel_Invalid_IsDuplicate] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_Parcel_Invalid_ID] PRIMARY KEY NONCLUSTERED ([ID] ASC) WITH (FILLFACTOR = 80)
);


GO
CREATE CLUSTERED INDEX [IX_Parcel_Invalid_Code]
    ON [dbo].[Parcel_Invalid]([Code] ASC) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [IX_Parcel_Invalid_LastModifiedDateUTC]
    ON [dbo].[Parcel_Invalid]([LastModifiedDateUTC] DESC) WITH (FILLFACTOR = 80);
