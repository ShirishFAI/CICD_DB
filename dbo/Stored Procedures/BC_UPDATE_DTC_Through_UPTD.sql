...
CREATE PROCEDURE [dbo].[BC_UPDATE_DTC_Through_UPTD]
AS
BEGIN
	/****************************************************************************************
-- AUTHOR		: Shirish W.
...
-- Step 1. Delete Address from DTC 
		...
		AND UPTD.Code IS NOT NULL

		SELECT UPTD.*
		INTO #Address_Update
		FROM #BC_UPTO_DATE UPTD
		INNER JOIN DBO.Address ADDR ON UPTD.Code = ADDR.Code
		WHERE 
		(
		...
		FROM Profisee.data.tSales tSales
		INNER JOIN Profisee.data.tSales tAddress ON tSales.Code = tAddress.Code
		WHERE tAddress.LastModifiedDateUTC = @StartDate;
