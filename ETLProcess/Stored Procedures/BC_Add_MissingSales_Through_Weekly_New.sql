...
CREATE PROCEDURE [ETLProcess].[BC_Add_MissingSales_Through_Weekly_New]
AS
BEGIN
	/****************************************************************************************
-- AUTHOR		: Shirish W.
...
******************************************************************************************/
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS #UPTD_Sales;
			...
			--WHERE WK.Sales_Sale_Action = 'Add';
			...
			AND NOT EXISTS 
				(
					SELECT 1 FROM DBO.BC_UPTO_DATE UPTD 
					WHERE WK.FolioRecord_ID = UPTD.FolioRecord_ID 
					AND WK.Sales_Sale_ID=UPTD.Sales_Sale_ID
				);
