CREATE procedure [ETLProcess].[PushToPermitEntity]      
...
-- VERSION HISTORY:
** ----------------------------------------------------------------------------------------
** 02/02/2022	Shirish Waghmale	Original Version
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToPermitEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to Permit Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to Permit Entity';
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('Permit') 
