...
CREATE procedure [ETLProcess].[PushToListingEntity]      
...
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToListingEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to Listing Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to Listing Entity';
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('Listing') 
