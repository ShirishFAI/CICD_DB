...
CREATE procedure [ETLProcess].[PushToPropertyEntity]      
...
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToPropertyEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to Property Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to Property Entity';
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('Property') 
				...
				INTO @EntityTableName;
					...
					BEGIN
						...
						BEGIN
							...
							--Inserts
							SET @strSQLInsert=@strSQLInsert+' INSERT INTO '+@EntityTableName+'('+'Code,'+@InsertTargetColumnsList+' ,DateCreatedUTC,LastModifiedDateUTC '+') SELECT s.Code,'+  @InsertSourceColumnsList+' ,'+@CurrentRunETLProcessUTC_StartedAt+', '+@CurrentRunETLProcessUTC_StartedAt+' ' +' FROM '+@StageProcessingTableName+' s LEFT JOIN '+@EntityTableName+ ' t ON s.Code=t.Code WHERE ActionType=''I'' AND t.Code IS NULL AND s.IsDuplicate=0 ;'
						...
						IF NOT EXISTS (SELECT 1 FROM #SourceColumnTargetColumnXref) AND @sourceRowCount>0
							...
							--Select @InsertTargetColumnsList,@InsertSourceColumnsList,@UpdateColumnsList
							--Inserts
							SET @strSQLInsert=@strSQLInsert+' INSERT INTO '+@EntityTableName+'('+'Code , DateCreatedUTC,LastModifiedDateUTC '+') SELECT s.Code ,'+@CurrentRunETLProcessUTC_StartedAt+', '+@CurrentRunETLProcessUTC_StartedAt+'' +' FROM '+@StageProcessingTableName+' s LEFT JOIN '+@EntityTableName+ ' t ON s.Code=t.Code WHERE ActionType=''I'' AND t.Code IS NULL AND s.IsDuplicate=0 ;'
							...
							--Updates
								...
								SET @strSQLUpdate=@strSQLUpdate+' UPDATE t SET  t.LastModifiedDateUTC =  '+@CurrentRunETLProcessUTC_StartedAt+''+' FROM '+@StageProcessingTableName+' s JOIN '+@EntityTableName+' t ON t.Code=s.Code Where s.ActionType=''U''  AND s.IsDuplicate=0 ;'
