...
CREATE procedure [ETLProcess].[PushToBuildingEntity]      
...
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToBuildingEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to Building Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to Building Entity';
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('Building') 
				...
				INTO @EntityTableName;
					...
					BEGIN
						...
						BEGIN
							...
							--Inserts
							--SET @strSQLInsert=@strSQLInsert+' INSERT INTO '+@EntityTableName+'('+'Code,'+@InsertTargetColumnsList+' ,Data_Source_ID, Data_Source_Priority,DateCreatedUTC,LastModifiedDateUTC '+') SELECT s.Code,'+  @InsertSourceColumnsList+' ,SUBSTRING(s.code, 1, CHARINDEX(''_'', s.code) -1) as Data_Source_ID, (Select Distinct Data_Source_Priority From ETLProcess.ETLProcess Where ProcessId= SUBSTRING(s.code, 1, CHARINDEX(''_'', s.code) -1)) As Data_Source_Priority,(Select Distinct UTC_StartedAt From ETLAudit.ETLProcessCategory Where RunID= SUBSTRING(s.Code, CHARINDEX(''_'', s.Code) + 1, CHARINDEX(''_'', s.Code, CHARINDEX(''_'', s.Code)+ 1)  - CHARINDEX(''_'', s.Code) - 1)) as DateCreatedUTC,(Select Distinct UTC_StartedAt From ETLAudit.ETLProcessCategory Where RunID= SUBSTRING(s.Code, CHARINDEX(''_'', s.Code) + 1, CHARINDEX(''_'', s.Code, CHARINDEX(''_'', s.Code)+ 1)  - CHARINDEX(''_'', s.Code) - 1)) as LastModifiedDateUTC ' +' FROM '+@StageProcessingTableName+' s LEFT JOIN '+@EntityTableName+ ' t ON s.Code=t.Code WHERE ActionType=''I'' AND t.Code IS NULL AND s.IsDuplicate=0 ;'
