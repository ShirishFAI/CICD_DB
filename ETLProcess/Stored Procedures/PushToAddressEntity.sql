...
CREATE procedure [ETLProcess].[PushToAddressEntity]      
...
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToAddressEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to Address Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to Address Entity';
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('Address') 
						...
						BEGIN
							...
							--Updates
								...
								WHERE 
									...
									AND COLUMN_NAME NOT IN('IsValid','ID','IsMADSent','MADSentDateUTC','IsMADReceived','MADReceivedDateUTC','MasterAddressID','LastModifiedDateUTC')
								ORDER BY 
									COLUMN_NAME;
								...
								SET @strSQLUpdate=@strSQLUpdate+' UPDATE t SET '+@UpdateColumnsList+',IsMADSent=NULL, MADSentDateUTC=NULL,IsMADReceived=NULL ,MADReceivedDateUTC = NULL,MasterAddressID=NULL ,t.LastModifiedDateUTC =  '+@CurrentRunETLProcessUTC_StartedAt+' FROM '+@StageProcessingTableName+' s JOIN '+@EntityTableName+' t ON t.Code=s.Code Where s.ActionType=''U''  AND s.IsDuplicate=0  ;'
