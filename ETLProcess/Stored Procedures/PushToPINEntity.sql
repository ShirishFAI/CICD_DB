...
CREATE procedure [ETLProcess].[PushToPINEntity]      
...
******************************************************************************************/
		...
		DECLARE @ProcessName VARCHAR(100)='PushToPINEntity';
		DECLARE @ProcessStage VARCHAR(100)='Push Records From StageProcessing to PIN Entity';
		DECLARE @ErroMessage VARCHAR(100)='Error Pushing Records From StageProcessing to PIN Entity';
		...
		Declare @strSQLUpdateOriginalPIN nvarchar(max)=N''
		...
		BEGIN
			...
			AND EXISTS(
					...
					SELECT DISTINCT 
						...
						FROM 
						INFORMATION_SCHEMA.COLUMNS c
						WHERE TABLE_SCHEMA='dbo' AND c.TABLE_NAME IN('PIN') 
					...
					BEGIN
						...
						BEGIN
							...
							--Updates
								...
								SET @HistoryStage =  'Start Orignal PIN Updates for '+@StageProcessingTableName;


								Declare @SourceHistoryTableName Varchar(200)=REPLACE(@StageProcessingTableName,'StageProcessing.','SourceHistory.')
						
								Declare @SourcePINColumn varchar(200)
								...
								SET @SourcePINColumn=(
													...
													FROM 
														INFORMATION_SCHEMA.COLUMNS c
														Join ETLProcess.ETLSourceMapping sm on sm.SourceColumnName=c.COLUMN_NAME
													Where
														c.TABLE_SCHEMA+'.'+c.TABLE_NAME=@SourceHistoryTableName
														AND sm.DestinationColumnName='PIN'
														)



								IF (@SourcePINColumn IS NOT NULL)
								BEGIN

								--SET @strSQLUpdateOriginalPIN=@strSQLUpdateOriginalPIN+' UPDATE e SET e.OriginalPIN=l.'+@SourcePINColumn+' FROM '+@EntityTableName+' e JOIN '+@SourceHistoryTableName+' l ON l.Code=e.Code AND HistEndDate IS NULL ;'

								SET @strSQLUpdateOriginalPIN=@strSQLUpdateOriginalPIN+' UPDATE e SET e.OriginalPIN=l.'+'['+@SourcePINColumn+']'+' FROM '+@EntityTableName+' e JOIN '+@SourceHistoryTableName+' l ON l.Code=e.Code JOIN '+@StageProcessingTableName+'  sp On sp.Code=l.Code  AND HistEndDate IS NULL ;'
							...
							EXEC ETLProcess.AuditLog
								...
								SET @HistoryStage =  'Completed Original PIN Updates for '+@StageProcessingTableName;

								EXECUTE sp_executesql @statement = @strSQLUpdateOriginalPIN
