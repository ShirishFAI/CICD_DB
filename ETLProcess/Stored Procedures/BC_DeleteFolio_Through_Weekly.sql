...
CREATE PROCEDURE [ETLProcess].[BC_DeleteFolio_Through_Weekly]
AS
BEGIN
	/****************************************************************************************
-- AUTHOR		: Shirish W.
...
******************************************************************************************/
	...
	IF OBJECT_ID('tempdb..#FolioDelete_BasicFileds') IS NOT NULL
		...
		IF OBJECT_ID('tempdb..#DTC_Code') IS NOT NULL
		DROP TABLE #DTC_Code;
	...
	BEGIN TRY
		...
		LEFT JOIN #BC_UPTO_DATE UPTD1 ON Address.FolioRecord_ID = UPTD1.FolioRecord_ID
			...
			INTO #DTC_Code
			FROM #Final Test
			LEFT JOIN #BC_UPTO_DATE BC ON ISNULL(BC.FolioAddresses_FolioAddress_City, '') = ISNULL		(Test.FolioAddresses_FolioAddress_City,  '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_PostalZip, '') = ISNULL(Test.FolioAddresses_FolioAddress_PostalZip, '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_ProvinceState, '') = ISNULL(Test.FolioAddresses_FolioAddress_ProvinceState, '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_StreetName, '') = ISNULL(Test.FolioAddresses_FolioAddress_StreetName, '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_StreetNumber, '') = ISNULL(Test.FolioAddresses_FolioAddress_StreetNumber, '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_StreetType, '') = ISNULL(Test.FolioAddresses_FolioAddress_StreetType, '')
				AND ISNULL(BC.FolioAddresses_FolioAddress_UnitNumber, '') = ISNULL(Test.FolioAddresses_FolioAddress_UnitNumber, '')
				AND ISNULL(BC.JurisdictionCode, '') = ISNULL(Test.JurisdictionCode, '')
				AND ISNULL(BC.LegalDescriptions_LegalDescription_FormattedLegalDescription, '') = ISNULL			(Test.LegalDescriptions_LegalDescription_FormattedLegalDescription, '')
				AND ISNULL(BC.LegalDescriptions_LegalDescription_PID, '') = ISNULL(Test.LegalDescriptions_LegalDescription_PID, '')
				AND ISNULL(BC.LegalDescriptions_LegalDescription_Range, '') = ISNULL(Test.LegalDescriptions_LegalDescription_Range, '')
				AND ISNULL(BC.LegalDescriptions_LegalDescription_Township, '') = ISNULL(Test.LegalDescriptions_LegalDescription_Township, '')
				AND ISNULL(BC.RollNumber, '') = ISNULL(Test.RollNumber, '')
				--AND ISNULL(BC.RollYear,'')	= ISNULL(Test.RollYear,'')
				AND ISNULL(BC.Sales_Sale_ConveyanceDate, '') = ISNULL(Test.Sales_Sale_ConveyanceDate, '')
				AND ISNULL(BC.Sales_Sale_ConveyancePrice, '') = ISNULL(Test.Sales_Sale_ConveyancePrice, '');
		...
		INNER JOIN DBO.Property Pr ON Pr.JurCode = DTC_Delete.JurisdictionCode
			AND Pr.ARN = DTC_Delete.RollNumber
			AND Pr.ProvinceCode = 'BC'
		INNER JOIN dbo.Address Address ON Address.Code = Pr.Code
		...
		FROM #Final DTC
		INNER JOIN DBO.Property Pr ON Pr.JurCode = DTC.JurisdictionCode
			AND Pr.ARN = DTC.RollNumber
			AND Pr.ProvinceCode = 'BC'
		LEFT JOIN dbo.Parcel Parcel ON Parcel.Code = Pr.Code
		...
		LEFT JOIN dbo.PIN PIN ON PIN.Code = Pr.Code AND PIN.Code = Parcel.Code
			AND ISNULL(PIN.PIN, '') = ISNULL(DTC.LegalDescriptions_LegalDescription_PID, '')
		LEFT JOIN dbo.Address Address ON Address.Code = Pr.Code AND Address.Code = Parcel.Code AND Address.Code = PIN.Code
		...
		INNER JOIN DBO.Property Pr ON Pr.JurCode = DTC.JurisdictionCode
			AND Pr.ARN = DTC.RollNumber
			AND Pr.ProvinceCode = 'BC'
		INNER JOIN dbo.Sales Sales ON Sales.Code = Pr.Code
		...
		INNER JOIN #DTC_Code DTC_Address ON Address.Code = DTC_Address.Code
		WHERE (
		...
		INNER JOIN #DTC_Code DTC_Address ON Building.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Business.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Listing.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Parcel.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Permit.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		SET ProvinceCode = NULL
			,LastModifiedDateUTC = @StartDate
		FROM dbo.PIN PIN
		INNER JOIN #DTC_Code DTC_Address ON PIN.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Property.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Taxation.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		INNER JOIN #DTC_Code DTC_Address ON Valuation.Code = DTC_Address.Code
		WHERE (
				DTC_Address.FolioAction_FolioDelete = '1'
				OR DTC_Address.FolioAddresses_FolioAddress_Action = 'Delete'
				);
		...
		FROM dbo.Address Address
		INNER JOIN #DTC_Code Legal ON Address.Code = Legal.Code
		WHERE (
		...
		SET PIN = NULL
			...
			,MasterAddressID = CASE WHEN Legal.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Building Building
		INNER JOIN #DTC_Code Legal ON Building.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		SET PIN = NULL
			...
			,MasterAddressID = CASE WHEN Legal.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Listing Listing
		INNER JOIN #DTC_Code Legal ON Listing.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		SET PIN = NULL
			...
			,MasterAddressID = CASE WHEN Legal.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Parcel Parcel
		INNER JOIN #DTC_Code Legal ON Parcel.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		SET PIN = NULL
			...
			,LastModifiedDateUTC = @StartDate
		FROM dbo.PIN PIN
		INNER JOIN #DTC_Code Legal ON PIN.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		SET PIN = NULL
			...
			,MasterAddressID = CASE WHEN Legal.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Property Property
		INNER JOIN #DTC_Code Legal ON Property.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		SET PIN = NULL
			...
			,MasterAddressID = CASE WHEN Legal.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Valuation Valuation
		INNER JOIN #DTC_Code Legal ON Valuation.Code = Legal.Code
		WHERE (
				Legal.FolioAction_FolioDelete = '1'
				OR Legal.LegalDescriptions_LegalDescription_Action = 'Delete'
				);
		...
		FROM dbo.Listing Listing
		INNER JOIN #DTC_Code Sales ON Listing.Code = Sales.Code
		WHERE (
		...
		SET PriceSold = NULL
			...
			,LastModifiedDateUTC = @StartDate
			,MasterAddressID = CASE WHEN Sales.FolioAction_FolioDelete = '1' THEN NULL ELSE MasterAddressID END
		FROM dbo.Sales DTC_Sale
		INNER JOIN #DTC_Code Sales ON DTC_Sale.Code = Sales.Code
		WHERE (
				Sales.FolioAction_FolioDelete = '1'
				OR Sales.Sales_Sale_Action = 'Delete'
				);
